...
public class JDHttpListener implements IHttpListener {
    ...
    public void processHttpMessage(int toolFlag, boolean messageIsRequest, IHttpRequestResponse messageInfo)
    {
        if (toolFlag == IBurpExtenderCallbacks.TOOL_SCANNER || toolFlag == IBurpExtenderCallbacks.TOOL_INTRUDER || toolFlag == IBurpExtenderCallbacks.TOOL_PROXY)
        {
            ...
            if (messageIsRequest && messageInfo != null && TSUtils.hasMagicHeader(messageInfo.getRequest(), helpers)) {
                ...
                byte[] newHTTPMessage = helpers.buildHttpMessage(headers, TSUtils.fromXML(body, helpers));
...
//            else if (toolFlag == IBurpExtenderCallbacks.TOOL_SCANNER && TSUtils.isJD(messageInfo.getResponse(), helpers))
            else if (!messageIsRequest && toolFlag == IBurpExtenderCallbacks.TOOL_SCANNER)
            {
                try {
                    byte[] XML = TSUtils.toXML(messageInfo.getResponse(), helpers);
